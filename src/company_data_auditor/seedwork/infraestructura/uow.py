...
from company_data_auditor.seedwork.dominio.entidades import AgregacionRaiz
...
def registrar_unidad_de_trabajo(serialized_obj):
    from company_data_collector.config.uow import UnidadTrabajoSQLAlchemy
    from flask import session
...
def flask_uow():
    from flask import session
    from company_data_collector.config.uow import UnidadTrabajoSQLAlchemy, UnidadTrabajoPulsar
    if session.get('uow'):
        return session['uow']
